---
name: drive
description: Turn one high-level goal into finished, verified work and drive it to completion autonomously. Classifies the work by shape, traits, and size, then researches, specifies, designs, plans tests, builds in parallel, verifies adversarially with independent agents, checks UI with vision, records state in files, and compounds lessons from failures. Scales from a one-line fix to a greenfield app. Only the user starts it.
...
- After compaction or a resume, drive's SessionStart hook prints the start view, then this file's
  section 1, Standing rules, section 6, and section 7 to the end. Re-read STATE.md and GOAL.md before
  acting on it.
