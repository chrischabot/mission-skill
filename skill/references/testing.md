...
| Weakened assertion | an exact matcher replaced by a looser one (`toEqual` to `toBeDefined`, `toContain`, or `toBeTruthy`; `assertEqual` to `assertTrue`, `assertIsNotNone`, or `assertTrue(True)`; `XCTAssertEqual` to `XCTAssertNotNil`; `assert x == y` to `assert x`); fewer expected items; a larger tolerance or a lower precision argument; an assertion wrapped in try with a broad catch that does not re-raise (a try with only finally, which closes a resource after the assertions, is not one), or a retry; an early `return` at the top of a test |
