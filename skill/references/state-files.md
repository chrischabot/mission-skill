...
`commit:<sha>`, `doc:<path>`, `why:<text>`, `planned:<path>::<name>` (only on a row below
Partial, and never at `lint --final` or in harden or a later phase), and `sub:<slug>`, which names the sub-goal a row belongs to when GOAL.md has sub-goals, so
